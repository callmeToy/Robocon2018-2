...
void setOption(byte option) {
  ...
  speedMode   = bitRead(option, 0);
  reverseMode = bitRead(option, 1);

  speedSet(speedMode);
  frontAngleSet(reverseMode);
}

void speedSet(bool speedMode) {
...
void frontAngleSet(int reverseMode) {
  if (reverseMode) {
    frontAngle = 90 + 180;
  } else {
    frontAngle = 90 + 0;
